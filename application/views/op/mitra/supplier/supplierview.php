		...
		<tbody>
			...
			if(!empty($data))
			{
				...
				<tr>
					...
					<td><?=$row->telepon;?></td>
					<td>						
						<a href="<?=base_url(akses().'/mitra/supplier/buatakun').'?id='.$id;?>" class="btn btn-xs btn-default"><i class="fa fa-user"></i></a>						
						<a href="<?=base_url(akses().'/mitra/supplier/edit').'?id='.$id;?>" class="btn btn-xs btn-info"><i class="fa fa-pencil"></i></a>
						<a onclick="return confirm('Yakin ingin menghapus supplier ini?');" href="<?=base_url(akses().'/mitra/supplier/delete').'?id='.$id;?>" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
					</td>
				</tr>
				<?php
				}
