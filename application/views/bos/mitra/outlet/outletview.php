		...
		<tbody>
			...
			if(!empty($data))
			{
				...
				<tr>
					...
					<td><?=$kota;?></td>
					<td>
						<a href="<?=base_url(akses().'/mitra/outlet/pengguna').'?id='.$id;?>" class="btn btn-xs btn-default"><i class="fa fa-users"></i></a>
						<a href="<?=base_url(akses().'/mitra/outlet/edit').'?id='.$id;?>" class="btn btn-xs btn-info"><i class="fa fa-pencil"></i></a>
						<a onclick="return confirm('Yakin ingin menghapus toko ini?');" href="<?=base_url(akses().'/mitra/outlet/delete').'?id='.$id;?>" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
					</td>
				</tr>
				<?php
				}
